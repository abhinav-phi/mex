...
random installation UUID. An installation is not a person or a team: two
computers have different IDs, and a shared installation has one ID.
...
creation and GeoIP enrichment are explicitly disabled.
...
raw arguments, receipt tokens, or Member/artifact IDs in events. Schema v2 also
omits the previous scaffold/project UUID. Unknown event properties and values
are rejected, including when reading queued events back from disk.

Random IDs allow repeat-use measurement; this is pseudonymous data, not a claim
of complete anonymity. Requests go to [PostHog Cloud](https://posthog.com), US
...
enable. A short-lived SQLite rollback journal may exist during writes. Nothing is stored in or shared through the
project's Git artifacts. `MEX_HOME` relocates the global `.mex` directory.
