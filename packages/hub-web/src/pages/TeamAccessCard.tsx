...
export function TeamAccessCard() {
  ...
  return (
    <>
      <Card className={homeStyles.updatesCard} role="region" aria-labelledby="overview-team-access-heading">
          ...
          {contactSent ? (
            ...
            <>
              ...
              <div className={homeStyles.updatesActions}>
                <Button onClick={() => { setStep("contact"); setOpen(true); }} size="sm" type="button">
                  Request access
                </Button>
              </div>
            </>
      ...
      <Dialog open={open} onOpenChange={closePanel}>
        <DialogContent className={cn(styles.dialog, "max-w-[min(460px,calc(100vw-32px))] gap-0 p-0 sm:max-w-[460px]")}>
          {step === "contact" ? (
            <form
