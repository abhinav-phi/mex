...
import type { SetupCommitPreview, SetupCommitResponse } from "./types";
...
const preview: SetupCommitPreview = {
  ...
  files: [{ path: ".mex/config.json", status: "added", diff: "+project identity", truncated: false }],
};
...
describe("setup commit transport", () => {
  it("protects both explicit POSTs with CSRF and sends only the reviewed revision and commit message", async () => {
    const fetch = vi.fn().mockResolvedValueOnce(json(session)).mockResolvedValueOnce(json(preview)).mockResolvedValueOnce(json(response));
    ...
    for (const call of [previewCall, commitCall]) {
