...
export async function startLocalIngestion() {
  ...
  return {
    ...
    begin(mode) { current = { mode, requests: 0, invalidBodies: 0, events: [] }; return current; },
    async finish() {
...
export async function inspectQueue(mexHome, receivedById = new Map()) {
  ...
  try {
    ...
    return { state: "available", ...row, databaseBytes, eventCounts, receivedOrQueuedEventCounts };
  } finally { database.close(); }
}

async function prepareModuleProbe(workspace) {
  ...
  try {
    await esbuild.build({
      stdin: { contents: `
import {captureEvent,flush,getTelemetryInspection,__resetTelemetryForTest} from ${JSON.stringify(source)};
import {performance} from 'node:perf_hooks';
const warmups=Number(process.argv[2]), samples=Number(process.argv[3]);
const results={started:[],completed:[],flush:[],firstStartedMs:null};
for(let index=0;index<warmups+samples;index++) {
  __resetTelemetryForTest();
  const start=performance.now();
  captureEvent('cli.command_started',{command:'commands',stage:'direct'});
  const middle=performance.now();
  captureEvent('cli.command_completed',{command:'commands',stage:'direct',outcome:'success',duration_ms:0});
...
export async function runTelemetryBenchmark({ cli, baseline, samples = 20, warmups = 5, progress = () => {} }) {
  ...
  try {
    ...
    const moduleProbePath = await prepareModuleProbe(workspace);
    const records = {};
    ...
    async function measure(build, command, condition, mexHome, ledger = { receivedById: new Map(), receivedEventCounts: {} }) {
      const received = ingestion.begin(condition === "healthy" || condition === "disabled" ? "healthy" : "hanging");
      let result;
      ...
      if (command.name === "failure" && (!result.stderr.trim() || result.stdout.trim())) throw new Error("Failure command output contract changed.");
      ...
      return { elapsedMs: result.elapsedMs, exitCode: result.code, stdoutBytes: Buffer.byteLength(result.stdout), stderrBytes: Buffer.byteLength(result.stderr), requests: received.requests, events: received.events.map(({ uuid: _uuid, ...event }) => event), openSocketsAfterExit: openSockets, ...result.diagnostics };
    }
    ...
    for (let round = 0; round < 3; round++) {
      for (const build of round % 2 ? [...builds].reverse() : builds) {
        for (const condition of ["disabled", "healthy"]) {
          ...
          const queue = await inspectQueue(mexHome);
    ...
    const groups = Object.fromEntries(await Promise.all(Object.entries(records).map(async ([key, record]) => [key, {
      ...
      queue: await inspectQueue(record.mexHome, record.receivedById),
    ...
    return {
      ...
      moduleProbe: { methodology: "Separately bundled current internal telemetry module; direct synchronous capture-call and awaited flush timings. Module/compiler startup excluded. Each repetition resets transport handles and keeps its isolated durable queue; first capture includes queue initialization. This does not replace built-CLI lifetime measurements.", bundleSha256: createHash("sha256").update(readFileSync(moduleProbePath)).digest("hex"), conditions: moduleProbe },
...
async function main() {
  ...
  for (let index = 0; index < args.length; index += 2) {
    if (!["--cli", "--baseline", "--output", "--samples", "--warmups"].includes(args[index]) || !args[index + 1]) throw new Error("Usage: node scripts/benchmark-telemetry.mjs --baseline <built-cli> --output <json> [--cli dist/cli.js] [--samples 20] [--warmups 5]");
    options[args[index].slice(2)] = args[index + 1];
  }
  if (!options.baseline || !options.output) throw new Error("A preserved baseline CLI and report output path are required.");
  const report = await runTelemetryBenchmark({ cli: options.cli ?? resolve(dirname(fileURLToPath(import.meta.url)), "../dist/cli.js"), baseline: options.baseline, samples: Number(options.samples ?? 20), warmups: Number(options.warmups ?? 5), progress: (value) => process.stderr.write(JSON.stringify(value) + "\n") });
