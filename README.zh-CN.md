...
> 此译文介绍的是 0.8.0 版本。正在开发的 0.8.1 新增了 Context 知识图谱、Inbox 知识贡献、面向团队开放的 Relay 和日志偏好设置；最新说明请参阅[英文 README](README.md)和[版本计划](docs/design/0.8.1-release-plan.md)。
...
MEX 具有**使用假名标识的 CLI 和 Hub 使用遥测**，默认启用，除非你选择退出。遥测记录命令名称和结果、Hub 的明确操作、页面类别及后台任务结果。两者共用一个随机安装 UUID，以衡量重复使用情况。不会发送项目标识符、参数值、路径、内容、搜索文本或联系方式。接收服务可以观察到常规传输元数据。事件使用有大小限制的本地队列；CLI 退出时只短暂等待，离线发送不会改变命令结果。
